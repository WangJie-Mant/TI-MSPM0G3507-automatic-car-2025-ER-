...
{
    if (*p_motor1_pwm > PWM_MAX)
        *p_motor1_pwm = PWM_MAX;
    if (*p_motor1_pwm < PWM_MIN)
        *p_motor1_pwm = PWM_MIN;

    if (*p_motor2_pwm > PWM_MAX)
        *p_motor2_pwm = PWM_MAX;
    if (*p_motor2_pwm < PWM_MIN)
        *p_motor2_pwm = PWM_MIN;
}
